...
func (s *ManagedClusterSpec) Parameters(ctx context.Context, existingObj genruntime.MetaObject) (params genruntime.MetaObject, err error) {
	...
	if existing != nil {
		...
		if s.UserAssignedIdentities != nil {
			uai := make(map[string]*containerservice.ManagedClusterIdentityUserAssignedIdentitiesValue)
			uai[s.UserAssignedIdentities[0].ProviderID] = &containerservice.ManagedClusterIdentityUserAssignedIdentitiesValue{
				// intentionally empty
			}
			if managedCluster.Identity == nil {
				managedCluster.Identity = &containerservice.ManagedClusterIdentity{UserAssignedIdentities: uai, Type: containerservice.ResourceIdentityType(infrav1.VMIdentityUserAssigned)}
			} else {
				managedCluster.Identity.Type = containerservice.ResourceIdentityType(infrav1.VMIdentityUserAssigned)
				managedCluster.Identity.UserAssignedIdentities = uai
			}
		}
...
func computeDiffOfNormalizedClusters(managedCluster *asocontainerservicev1hub.ManagedCluster, existingMC containerservice.ManagedCluster) string {
	...
	if managedCluster.Spec.AadProfile != nil {
		propertiesNormalized.AadProfile = &containerservice.ManagedClusterAADProfile{
			Managed:             managedCluster.Spec.AadProfile.Managed,
			EnableAzureRBAC:     managedCluster.Spec.AadProfile.EnableAzureRBAC,
			AdminGroupObjectIDs: managedCluster.Spec.AadProfile.AdminGroupObjectIDs,
		}
	}
	...
	if managedCluster.Spec.NetworkProfile != nil {
	...
	if managedCluster.Spec.Sku != nil {
		clusterNormalized.Sku = managedCluster.Spec.Sku
	}
	...
	if managedCluster.Identity != nil {
		clusterNormalized.Identity = managedCluster.Identity
	}

	if existingMC.Identity != nil {
		existingMCClusterNormalized.Identity = existingMC.Identity
	}
